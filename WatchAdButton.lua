-- WatchAdButton.client.lua
-- Place this LocalScript inside the "Watch Ad" button for 2x SPEED boost
...
if not button or not (button:IsA("TextButton") or button:IsA("ImageButton")) then
	warn("[WatchAdButton] Could not find a clickable TextButton or ImageButton!")
	return
end

-- SPEED BOOST SETTINGS
local PRODUCT_ID = 3534035402 -- 2x Speed dev product
...
local function checkAdAvailability()
	...
	if success and result.AdAvailabilityResult == Enum.AdAvailabilityResult.IsAvailable then
		isAdAvailable = true
		print("[WatchAdButton] ✅ Speed ads available")
	else
		isAdAvailable = false
		print("[WatchAdButton] ❌ Speed ads not available:", result and result.AdAvailabilityResult or "Failed to check")
	end
end
...
button.MouseButton1Click:Connect(function()
	...
	shrinkTween.Completed:Connect(function()
		...
		print("[WatchAdButton] Requesting server to show ad for", BOOST_TYPE)
		...
		if success then
			print("[WatchAdButton] Server response:", result)
			...
			warn("[WatchAdButton] Failed to invoke server:", result)
			if starterGui then
				starterGui:SetCore("SendNotification", {
					Title = "Ad Error",
...
print("[WatchAdButton] Initialized for Speed boost")
print("[WatchAdButton] Button visibility: Shows only when ads available AND no active boost")
