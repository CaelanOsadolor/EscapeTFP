--[[
  WinsLeaderboardClass, Based on TimePlayedClass by RenanMSV @2023

  A script designed to update a leaderboard with
  the top 10 players who have the most wins.
...
local Config = require(script.Parent.WinsSettings)

local WinsLeaderboardClass = {}
WinsLeaderboardClass.__index = WinsLeaderboardClass


function WinsLeaderboardClass.new()
	local new = {}
	setmetatable(new, WinsLeaderboardClass)
...
function WinsLeaderboardClass:_init()
	...
	if self._isMainScript then
		...
		self._apiServicesEnabled = (ServerStorage:WaitForChild("WinsLeaderboard_NoAPIServices_Flag", 99) :: BoolValue).Value
	...
	-- Sync wins from SaveManager to OrderedDataStore every minute
	task.spawn(function ()
		if not self._isMainScript then return end
		while true do
			task.wait(60) -- Update every minute
			self:_syncWinsToOrderedDataStore()
		end
	end)
...
function WinsLeaderboardClass:_clearBoard ()
...
function WinsLeaderboardClass:_syncWinsToOrderedDataStore()
	-- Get SaveManager and sync current players' wins
	local SaveManager = nil
	local suc = pcall(function()
		SaveManager = require(game:GetService("ServerScriptService"):WaitForChild("GameManager"):WaitForChild("SaveManager"))
	end)

	if suc and SaveManager then
		for _, player in pairs(game:GetService("Players"):GetPlayers()) do
			local wins = SaveManager.GetWins(player)
			if wins and wins > 0 then
				pcall(function()
					local stat = self._dataStoreStatName .. player.UserId
					self._datastore:SetAsync(stat, wins)
					if self._doDebug then print("Synced", player.Name, "wins:", wins) end
				end)
			end
		end
	end
...
function WinsLeaderboardClass:_updateBoard ()
	if self._doDebug then print("Updating Wins board") end
	local results = nil
	...
	if not suc or not results then
		if self._doDebug then warn("Failed to retrieve top 10 with most wins. Error:", results) end
		return
	end
	...
	for k, v in pairs(results) do
		...
		local score = self:_winsToString(v.value)
	...
	if self._doDebug then print("Wins board updated sucessfully") end
end


function WinsLeaderboardClass:_onPlayerScoreUpdate (userid, wins)
	...
	if leaderstat then
		leaderstat.Value = tonumber(wins)
	end
end


function WinsLeaderboardClass:_checkDancingRigEnabled()
...
function WinsLeaderboardClass:_checkIsMainScript()
	local winsLeaderboardRunning = ServerStorage:FindFirstChild("WinsLeaderboard_Running_Flag")
	if winsLeaderboardRunning then
		self._isMainScript = false
	else
		self._isMainScript = true
		local boolValue = Instance.new("BoolValue", ServerStorage)
		boolValue.Name = "WinsLeaderboard_Running_Flag"
		boolValue.Value = true
	end
end


function WinsLeaderboardClass:_checkDataStoreUp()
	local status, message = pcall(function()
		-- This will error if current instance has no Studio API access:
		DataStoreService:GetDataStore("____PS_Wins"):SetAsync("____PS", os.time())
	end)
	if status == false and
		...
		boolValue.Name = "WinsLeaderboard_NoAPIServices_Flag"
		return false
	end
...
function WinsLeaderboardClass:_winsToString(wins)
	if wins >= 1000000 then
		return string.format("%.1fM Wins", wins / 1000000)
	elseif wins >= 1000 then
		return string.format("%.1fK Wins", wins / 1000)
	else
