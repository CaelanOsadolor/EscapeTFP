--[[
  MoneyLeaderboardClass
  
  A script designed to update a leaderboard with
  the top 10 players who have the most money.
  
  Connects to SaveManager's PlayerData_V1 datastore and leaderstats.
]]

local DataStoreService = game:GetService("DataStoreService")
local ServerStorage = game:GetService("ServerStorage")

local Config = require(script.Parent.MoneyLeaderboardSettings)

local MoneyLeaderboardClass = {}
MoneyLeaderboardClass.__index = MoneyLeaderboardClass


function MoneyLeaderboardClass.new()
	local new = {}
	setmetatable(new, MoneyLeaderboardClass)
...
function MoneyLeaderboardClass:_init()
	...
	if self._isMainScript then
		...
		self._apiServicesEnabled = (ServerStorage:WaitForChild("MoneyLeaderboard_NoAPIServices_Flag", 99) :: BoolValue).Value
	...
	-- Sync money from leaderstats to OrderedDataStore every minute
	task.spawn(function ()
		if not self._isMainScript then return end
		while true do
			task.wait(60) -- Update every minute
			self:_syncMoneyToOrderedDataStore()
		end
	end)
...
function MoneyLeaderboardClass:_clearBoard ()
...
function MoneyLeaderboardClass:_syncMoneyToOrderedDataStore()
	-- Sync current players' money from leaderstats
	for _, player in pairs(game:GetService("Players"):GetPlayers()) do
		local leaderstats = player:FindFirstChild("leaderstats")
		if leaderstats then
			local money = leaderstats:FindFirstChild("Money")
			if money and money.Value and money.Value > 0 then
				pcall(function()
					local stat = self._dataStoreStatName .. player.UserId
					self._datastore:SetAsync(stat, math.floor(money.Value))
					if self._doDebug then print("Synced", player.Name, "money:", money.Value) end
				end)
			end
		end
	end
...
function MoneyLeaderboardClass:_updateBoard ()
	if self._doDebug then print("Updating Money board") end
	local results = nil
	...
	if not suc or not results then
		if self._doDebug then warn("Failed to retrieve top 10 with most money. Error:", results) end
		return
	end
	...
	for k, v in pairs(results) do
		...
		local score = self:_moneyToString(v.value)
	...
	if self._doDebug then print("Money board updated sucessfully") end
end


function MoneyLeaderboardClass:_onPlayerScoreUpdate (userid, money)
	...
	if leaderstat then
		-- FIX: Never overwrite leaderstats from OrderedDataStore - OrderedDataStore is for DISPLAY only!
		-- The OrderedDataStore can be stale (updates every minute), so writing it back to 
		-- leaderstats causes money to revert to old values at trillion+ scale.
		-- PlayerSetup and SaveManager are the ONLY sources of truth for player money.
		-- leaderstat.Value = tonumber(money)  -- REMOVED - causes bug at trillion scale
	end
end


function MoneyLeaderboardClass:_checkDancingRigEnabled()
...
function MoneyLeaderboardClass:_checkIsMainScript()
	local moneyLeaderboardRunning = ServerStorage:FindFirstChild("MoneyLeaderboard_Running_Flag")
	if moneyLeaderboardRunning then
		self._isMainScript = false
	else
		self._isMainScript = true
		local boolValue = Instance.new("BoolValue", ServerStorage)
		boolValue.Name = "MoneyLeaderboard_Running_Flag"
		boolValue.Value = true
	end
end


function MoneyLeaderboardClass:_checkDataStoreUp()
	local status, message = pcall(function()
		-- This will error if current instance has no Studio API access:
		DataStoreService:GetDataStore("____PS_Money"):SetAsync("____PS", os.time())
	end)
	if status == false and
		...
		boolValue.Name = "MoneyLeaderboard_NoAPIServices_Flag"
		return false
	end
...
function MoneyLeaderboardClass:_moneyToString(money)
	-- Extended suffix system matching the rest of the game
