-- Admin Money & Speed Giver with Chat Commands
...
local ALL_THINGS = {
	...
	Uncommon = {"iPhone 7", "RealMe C67", "Samsung S7", "Samsung S8", "Samsung Tab"},
	Rare = {"Huawei Y6", "iPhone 8", "Samsung A50", "Samsung Note8", "Samsung S10"},
	Epic = {"iPhone 9", "iPhone Bronze", "OnePlus 12", "RealMe GT6", "Samsung Tab S"},
	Legendary = {"Huawei Nova Flip", "iPhone Silver", "iPhone XR", "Oppo A17", "Samsung S20"},
	Mythical = {"Huawei Mate 50", "iPhone 11", "iPhone Gold", "iPhone 13"},
	Divine = {"iPhone 14 Pro", "iPhone 15 Pro", "iPad 4th Gen", "iPhone 17"},
	Secret = {"iPhone 13 Pro", "Samsung S25 Ultra", "iPad 6th Gen", "iPad 9th Gen", "iPhone 16 Pro"},
	Celestial = {"iPhone 17 Pro", "Samsung Z Flip", "iPad Pro", "iPhone 17 Pro Max"},
	Limited = {"Valentine Pro Max"}
}
	...
	for _, player in pairs(Players:GetPlayers()) do
		if player.Name == playerName then
			...
			if leaderstats then
				...
				if money then
					print("[GiveMoney:17] Adding money via command:", moneyAmount, "| Current:", money.Value, "| New:", money.Value + moneyAmount)
	...
	player.Chatted:Connect(function(message)
		...
		elseif lowerMsg:match("^/givemoney%s+%d+$") then
			...
			if amount then
				...
				if leaderstats then
					local money = leaderstats:FindFirstChild("Money")
					if money then
						print("[GiveMoney:145] Adding money via /givemoney command:", amount, "| Current:", money.Value, "| New:", money.Value + amount)
		...
		elseif lowerMsg == "/resetall" then
			...
			if leaderstats then
				local money = leaderstats:FindFirstChild("Money")
				if money then
					print("[GiveMoney:168] Resetting money to 0 via /resetall for", player.Name, "| Current:", money.Value)
					money.Value = 0
				end
			end
...
print("Admin command system loaded!")
print("Commands:")
