	...
	if handle and handle:IsA("BasePart") then
		...
		if statsGui then
			...
			if frame then
				...
				-- Check for active event and apply event mutations first
				...
				if mutationLabel and mutationLabel:IsA("TextLabel") then
					...
					if activeEvent then
						-- 5% chance for event mutation during events
						if math.random() < 0.05 then
							if activeEvent == "Night" then
								mutationType = "Night"
								mutationLabel.Text = "Night"
							elseif activeEvent == "Love" then
								mutationType = "Love"
					...
					-- If no event mutation, try regular mutations
