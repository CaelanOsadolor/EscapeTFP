...
function BaseSlotManager.OnStealTriggered(stealerPlayer, victimPlayer, slot)
	...
	local productId = 3515273539 -- Steal product (49 Robux)
...
function BaseSlotManager.PlaceThing(player, thing, slot)
	...
	-- Apply slot-specific rotation based on slot number
	...
	local slotRotation = CFrame.Angles(0, math.rad(rotationAngle), 0)
	local finalPosition = Vector3.new(placeHolder.Position.X, currentY + yOffset, placeHolder.Position.Z)
	thing:PivotTo(CFrame.new(finalPosition) * slotRotation * originalRotation)

	-- Anchor all parts and setup collision, ensure visual elements stay visible
	for _, part in ipairs(thing:GetDescendants()) do
		if part:IsA("BasePart") then
			part.Anchored = true
			part.CanCollide = false
			-- DON'T modify transparency at all - preserve original transparency from template
			-- Phones and other models rely on their original transparency values
		elseif part:IsA("Decal") or part:IsA("Texture") then
			-- CRITICAL: Phones use Decals/Textures! Ensure they stay fully visible
			part.Transparency = 0
		elseif part:IsA("SpecialMesh") then
			-- CRITICAL: Some phones use SpecialMesh! Ensure TextureId is preserved
			-- Don't modify mesh properties, they need to stay intact
		elseif part:IsA("SurfaceGui") then
			-- Keep SurfaceGuis enabled (except StatsGui which we handle separately)
			if part.Name ~= "StatsGui" then
				part.Enabled = true
			end
			...
			if touchedPlayer == player then
				...
				if accumulated > 0 then
					...
					if leaderstats then
						...
						if money then
							print("[BaseSlotManager:954] Adding accumulated money:", accumulated, "| Current:", money.Value, "| New:", money.Value + accumulated)
		...
		if statsGui then
			...
			if frame then
				...
				if nameLabel then
					...
					if thing.Name == "Valentine Pro Max" then
...
function BaseSlotManager.LoadPlacedThings(player)
	...
	for slotNumber, thingData in pairs(data.PlacedThings) do
		...
		if slot then
			...
			warn("[LoadPlacedThings] Could not find slot:", slotName, "for", player.Name)
		end
	end
